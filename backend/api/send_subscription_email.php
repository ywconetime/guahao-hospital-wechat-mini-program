...
$userWechat = $data['user_wechat'] ?? '';
$userEmail = $data['user_email'] ?? '';

if (empty($orderNo) || empty($userPhone)) {
    echo json_encode([
        'code' => 400,
        'message' => '缺少必要参数'
    ], JSON_UNESCAPED_UNICODE);
...
try {
    $result = sendSubscriptionEmail($orderNo, $planName, $price, $userName, $userPhone, $userWechat, $userEmail);
...
function sendSubscriptionEmail($orderNo, $planName, $price, $userName, $userPhone, $userWechat, $userEmail)
{
    // 从授权系统获取邮件配置
    ...
    $subject = $siteName . ' - 新订阅套餐预约';
    $message = "<html><body>";
    $message .= "<h2 style='color: #667eea;'>" . $siteName . " - 新订阅套餐预约</h2>";
    $message .= "<p style='font-size: 16px; color: #333;'>有用户提交了订阅套餐预约！</p>";
    $message .= "<table border='1' cellpadding='10' cellspacing='0' style='border-collapse: collapse; width: 100%;'>";
    $message .= "<tr><th style='background: #667eea; color: white; text-align: left; width: 150px;'>项目</th><th style='background: #667eea; color: white; text-align: left;'>信息</th></tr>";
    $message .= "<tr><td>用户姓名</td><td>" . htmlspecialchars($userName ?: '未填写') . "</td></tr>";
    $message .= "<tr><td>手机号</td><td>" . htmlspecialchars($userPhone) . "</td></tr>";
    $message .= "<tr><td>微信号</td><td>" . htmlspecialchars($userWechat ?: '未填写') . "</td></tr>";
    $message .= "<tr><td>邮箱</td><td>" . htmlspecialchars($userEmail ?: '未填写') . "</td></tr>";
    $message .= "<tr><td>套餐名称</td><td>" . htmlspecialchars($planName) . "</td></tr>";
    $message .= "<tr><td>套餐价格</td><td style='color: #d63384; font-weight: 600;'>¥" . number_format($price, 2) . "</td></tr>";
    $message .= "<tr><td>订单号</td><td>" . htmlspecialchars($orderNo) . "</td></tr>";
    $message .= "<tr><td>提交时间</td><td>" . date('Y-m-d H:i:s') . "</td></tr>";
    $message .= "</table>";
    $message .= "<p style='margin-top: 20px; color: #666; font-size: 14px;'>请尽快联系用户确认并完成授权！</p>";
    $message .= "</body></html>";
    ...
    try {
        file_put_contents(__DIR__ . '/email_error_log.txt', date('Y-m-d H:i:s') . " - 开始发送订阅邮件...\n", FILE_APPEND);
        ...
        file_put_contents(__DIR__ . '/email_error_log.txt', date('Y-m-d H:i:s') . " - 订阅邮件发送成功！\n", FILE_APPEND);
        ...
        file_put_contents(__DIR__ . '/email_error_log.txt', date('Y-m-d H:i:s') . " - 发送订阅邮件异常: " . $e->getMessage() . "\n", FILE_APPEND);
